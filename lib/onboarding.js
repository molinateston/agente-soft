...
//
// Fluxo refinado (23/07/2026):
//   1) DM · dono manda a 1ª msg → agente abre com UMA pergunta só sobre o negócio.
//   2) DM · dono responde → agente interpreta segmento e volta com SUGESTÃO CONCRETA
//      de 4-6 tópicos que fazem sentido pra ESSE negócio, mais passo-a-passo pra criar
//      um grupo no Telegram e adicionar o agente como admin.
//   3) GRUPO · dono manda /prontos → agente usa a API createForumTopic pra criar cada
//      tópico ali, salva em topics.json, confirma no próprio grupo.
//   4) GRUPO · dono ajusta em linguagem natural ("troca X por A", "adiciona um de Y",
//      "tira Z"). Quando escrever "fechou" (ou "pronto"/"beleza"), o agente conclui.
//   5) Persistência: .onboarding-done é gravado no fim. /reonboarding apaga e refaz.
//
// Uso no bridge.cjs:
//   const onboarding = require("./lib/onboarding.js");
//   if (isOwner && !onboarding.isDone(__dirname)) {
//     const handled = await onboarding.handle({
//       workdir: __dirname, chatId, threadId, isGroup, text, send
//     });
//     if (handled) continue;
//   }
...
1. Cria um grupo novo no Telegram (Nova mensagem > Grupo)
2. Coloca um nome qualquer e me adiciona no grupo
3. Vai em "Editar grupo" > "Administradores" > "Adicionar administrador" > me seleciona (deixa "Gerenciar Tópicos" ligado)
4. Depois volta pro grupo e manda /prontos ali dentro. Eu crio as salas pra gente na hora.

Se quiser trocar nome ou adicionar tema, a gente ajusta depois que eu criar.`;
}

function reprontosHelp() {
  return `Manda /prontos DENTRO do grupo, com o bot já adicionado como admin. Aí eu crio as salas ali.`;
}
...
function confirmMsg(criados, falhas) {
  ...
  if (falhas.length) {
    corpo += `\n\nEssas eu não consegui criar: ${falhas.join(", ")}. Provável que eu não esteja como admin com permissão de "Gerenciar Tópicos". Confere aí e me avisa.`;
  }
...
async function handle({ workdir, chatId, threadId, isGroup, text, send }) {
  ...
  // STEP 3 · awaiting_prontos: /prontos no grupo dispara criação
  if (state.step === "awaiting_prontos") {
    if (/^\/prontos\b/i.test(raw)) {
      if (!isGroup) {
        await sendSafe(send, chatId, reprontosHelp(), threadId);
        return true;
      }
      const sug = state.sugestao || ["Comercial", "Financeiro", "Ideias", "Testes"];
      const criados = [];
      const falhas = [];
      const topicos = [];
      ...
      writeState(workdir, {
        step: criados.length ? "awaiting_adjustments" : "awaiting_prontos",
    ...
    // qualquer outra coisa na DM enquanto espera /prontos: reforça
    if (!isGroup) {
      await sendSafe(send, chatId, `Assim que criar o grupo e me adicionar como admin, manda /prontos ali dentro que eu crio as salas.`, threadId);
      return true;
    }
    return false;
  }

  // STEP 4 · awaiting_adjustments: ajustes ou conclusão no grupo
  if (state.step === "awaiting_adjustments") {
    if (!isGroup) return false;
    if (String(chatId) !== state.groupId) return false;
    ...
    if (parsed.tipo === "adicionar") {
      try {
        ...
        await sendSafe(send, chatId, `Não consegui criar "${parsed.nome}": ${e.message}. Confere se eu tô como admin com "Gerenciar Tópicos" ligado.`, threadId);
      }
      return true;
    }
    if (parsed.tipo === "trocar") {
      ...
      try {
        ...
        await sendSafe(send, chatId, `Não consegui renomear: ${e.message}.`, threadId);
      }
      return true;
    }
    if (parsed.tipo === "remover") {
      ...
      try {
        ...
        await sendSafe(send, chatId, `Não consegui tirar: ${e.message}.`, threadId);
      }
      return true;
    }
  }
...
module.exports = { handle, isDone, reset };
