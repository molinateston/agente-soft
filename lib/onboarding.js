...
// Este arquivo chega por update nas máquinas que JÁ rodavam. Sem esta guarda, o dono que
// usa o agente há semanas recebe "acabei de instalar aqui" do nada. Dois sinais de que a
// instalação é antiga: já tem salas mapeadas, ou a pasta nasceu bem antes deste arquivo.
function instalacaoAntiga(workdir) {
  try { if (Object.keys(readTopics(workdir)).length) return true; } catch {}
  try {
    const nasceu = fs.statSync(workdir).birthtimeMs || 0;
    const chegou = fs.statSync(__filename).mtimeMs || 0;
    if (nasceu && chegou && chegou - nasceu > 24 * 60 * 60 * 1000) return true;
  } catch {}
  return false;
}
...
function resgateMsg(state) {
  const topicos = (state && state.sugestao) || [];
  const linhas = topicos.map(n => {
    const h = topicHint(n);
    return h ? `• ${n}: ${h}` : `• ${n}`;
  }).join("\n");
  const plano = linhas ? `Teu plano de salas continua guardado:\n\n${linhas}\n\n` : "";
  return `Mudei uma coisa importante aqui: não existe mais comando nenhum pra decorar. Pode esquecer aquele que eu pedi antes.

${plano}O grupo virou opcional. Você escolhe:

1. Continuar aqui nesta conversa. Funciona inteiro, do mesmo jeito. Nada muda.

2. Usar um grupo com as salas separadas. Se você JÁ criou o grupo e já me colocou como administrador, é só me mandar qualquer mensagem lá dentro que eu crio as salas na hora.

Se ainda não criou, são 3 passos:

a) Toca no lápis de escrever, "Novo grupo", me adiciona e dá o nome que quiser.
b) Dentro do grupo, toca no nome dele lá em cima, "Editar", e liga a chave "Tópicos".
c) Ainda em "Editar", vai em "Administradores", "Adicionar administrador" e me escolhe.

Sem a chave "Tópicos" ligada o Telegram não me deixa criar sala nenhuma. É esse passo que costuma faltar.
...
async function handle({ workdir, chatId, threadId, isGroup, text, send }) {
  ...
  // dono antigo que só agora recebeu o onboarding: fecha calado, nunca se apresenta
  if (!readState(workdir) && instalacaoAntiga(workdir)) { markDone(workdir); return false; }
  ...
  // Resgate de quem ficou preso na versão antiga (a que exigia o comando /prontos):
  // manda a orientação nova UMA vez, dizendo que o comando morreu, e fecha.
  if (state.step === "awaiting_prontos" || state.step === "awaiting_adjustments") {
    writeState(workdir, Object.assign({}, state, { step: "plan_ready" }));
    markDone(workdir);
    await sendSafe(send, chatId, resgateMsg(state), threadId);
    return true;
  }

  // Qualquer outro estado adiante já teve a orientação. Fecha e devolve o turno
  // pro agente conversar normal.
  markDone(workdir);
  return false;
}
