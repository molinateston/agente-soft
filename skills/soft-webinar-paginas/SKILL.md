...
- **Doc enxuto pros 2 leitores (Lei 6):** o que sai serve o humano que cola E a IA que recebe a página como contexto. Só os blocos da página + os `[A CONFIRMAR: o que falta]` + os rótulos mínimos pra navegar. Zero meta-narração ("isto é o bloco que faz X"), zero bastidor, zero explicação-do-método-pro-leitor.
...
- Você **nunca inventa fala, número, prova ou case do cliente** (Lei 5); sem fonte, marca `[A CONFIRMAR: o que falta]` (dois-pontos, nunca travessão dentro do marcador) no lugar exato do furo e não conta como ancorado. Insumo que falta é pendência declarada, jamais buraco preenchido com algo plausível.
- Você **nunca mostra página que falhou no gate**.


## ⚠️ ENTREGA = UM doc MD, SEMPRE (nunca pingar a peça no chat)
Regra dura, vale mesmo pra copy curta: o RESULTADO desta skill sai como **UM documento markdown consolidado**. No **claude.ai**, um **artifact de markdown** (o dono abre, copia, baixa); no **Claude Code**, um arquivo `.md`; no **agente/Telegram**, gera o doc como arquivo e cita o path completo na resposta (vira anexo), com a condução em mensagens curtas, sem markdown pesado (sem `##`, sem tabela `|` no texto ao usuário). A CONDUÇÃO (perguntas de contexto, escolhas, os STOPs de aprovação) acontece no chat; a PEÇA/COPY em si mora no DOC. Ao parar num STOP, você mostra ou atualiza o DOC e pergunta "ajusto?"; você NUNCA reescreve a peça em pedaços no corpo da conversa. Sem o doc entregue, a skill não terminou.
...
- **PERPÉTUO** (sessão que roda sempre, sem replay): a página **não tem data fixa** escrita, cada lead vê a SUA sessão/horário/link (a espinha do Passo 5). Cadastro mostra horário **relativo** ("próxima sessão começa em 12 min", just-in-time) ou horários fixos do dia (recurring); o anti-fuga usa a fórmula HONESTA do perpétuo, **"essa sessão não terá replay"** (verdadeiro de fato: desliga o replay instantâneo na ferramenta), NUNCA "ao vivo, sem gravação" cru num gravado, que tangencia o guardrail de honestidade. Timer ao vivo quando a sessão começa em <60 min. Captura o **horário escolhido** no form (vira variável da automação).
...
> 5. **Qual o número real, preço e vagas?** (confirmado de fato, não estimado, número que você não bate de verdade vira `[A CONFIRMAR: nº/preço/vagas]`)
...
**1. Hook/Headline** (a peça mais importante, prende em 2s). Fôrma-mãe: "Como usar [mecanismo único] pra [objetivo final] mesmo sem [a principal objeção]" (+ opcional "em [tempo]"). Cabe em 1 linha no mobile (≤14 palavras), específica (não "ganhe mais"). O nome do mecanismo é SLOT do cliente; usa o que existe ou marca `[A CONFIRMAR: nome do mecanismo]`. Se a variação escolhida for a "promessa primária direta" (mecanismo opcional na headline) e o mecanismo ficou pendente, ele **reaparece marcado `[A CONFIRMAR: nome do mecanismo]` ao menos uma vez** na subheadline ou nos bullets, nunca some da página sem deixar rastro do furo. Troca os três campos da fôrma pelo nicho, **nunca copia headline alheia, copia a fôrma**.
  ...
  **Escolhe 1 das 4 variações pela TEMPERATURA do avatar** (não tem variação "padrão", a escolha é estratégica):
  ...
  Avatar cético: começa com "Por que"/"O que" em vez de "Como", que não soa promessa fácil. Headline instanciada (exemplo ilustrativo, nicho fictício; modela a qualidade, nunca copia): "Como dobrar a produtividade do seu time sem contratar mais ninguém" (mecanismo = método · objetivo = dobrar produtividade · objeção = contratar mais gente). Repara que os três campos da fôrma trocam pelo nicho. **Lê `references/paginas-cadastro-obrigado-checkout.md` Bloco 1.1 e aplica os 4 moldes instanciados em nichos diferentes** (emprego, comportamento felino, gestão, posicionamento), pra ver a mesma fôrma virar 4 headlines reais e instanciar a do nicho atual.

**2. Subheadline** (opcional, recomendada): explica a promessa em 1-2 frases. Tem **duas funções, escolhe uma**: reforço por **urgência** ("saiba o que ajustar hoje mesmo") OU reforço por **prova social** ("a estratégia que já recolocou mais de mil pessoas"). Molde: "Webinar gratuito de [duração]. Você sai com [resultado tangível] e o método completo, sem enrolação." Sem número de prova confirmado, vira `[A CONFIRMAR: nº de prova]`.
...
**4. Foto/vídeo + autoridade breve** (no TOPO): 2-3 linhas só, não o currículo. Empatia ANTES do feito. A bio DETALHADA não vai aqui, vai na ÚLTIMA DOBRA (Passo 1.9, regra das 3 páginas). Molde topo: "[Nome]. [empatia: já passei por X]. [feito: hoje Y clientes têm Z]." Número só confirmado; sem fonte, `[A CONFIRMAR: nº de clientes]`.
...
**7. Contra-filtro** (a parte mais Soft): "NÃO é pra você se… / É pra você se…". Diz pra quem NÃO é antes de pra quem é. **Mantém SEMPRE a linha anti-milagre no NÃO** (expulsa quem busca fórmula mágica / viralização / atalho); essa é a peça universal que não se troca por nicho. Promessa Soft: simples e honesto, nunca fácil e mágico. Quem não é avatar fecha a aba (ótimo, ele só sujaria a métrica); quem é, confirma "é pra mim" e entra mais comprometido.

  Molde do contra-filtro (exemplo ilustrativo, nicho fictício; modela a qualidade, nunca copia):
  > NÃO é pra você se: procura uma ferramenta mágica que resolve sozinha (linha anti-milagre) · não tem produto/serviço definido · acha que o problema é só falta de motivação.
  > É pra você se: já fatura com seu serviço e quer estruturar pra escalar · está cansado de trabalhar no escuro · quer um método claro pra atrair cliente pronto.

  A linha anti-milagre no NÃO é a única que não se troca por nicho. **Lê Bloco 1.7 e aplica o molde NÃO-é/É + os dois exemplos (Soft Business e gestão) com a linha anti-milagre marcada.**
...
| Check | Passa se | ✓/✗ |
|---|---|---|
| **Ancorada no verbatim** | promessa/bullets/prova nascem de fala ou prova REAL do cliente (cita N real); número/case/fala inventado = ✗ automático (vira `[A CONFIRMAR: o que falta]`) | |
